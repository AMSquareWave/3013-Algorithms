 ...
 *      Enter Key           = 10
 *      Space Bar           = 32
 *      Arrow Keys          = 27 (Esc), 91 ([), 65 -> 68 (A -> D)
 *
...
vector<string> loadData(string file_name) {
    ...
    fin.open("./data/dictionary.txt"); // open file for reading
    ...
    while ((!found) && (k = getch()) != 'Z') {
        ...
        // tests for an escape sequence or the escape key
            ...
            if ((k = getch()) == 91 && (k = getch()) == 65 || k == 66) {
                ...
                if (k == 65) {
                    ...
                    cout << termcolor::on_bright_red << termcolor::bright_white << "up" << termcolor::reset << endl;
                } else {
                    down = true;
                    highlight = (highlight+1) % matches.size();
        ...
        if ((int)k != 32) { // if k is not a space print it
            ...
            for (int i = 0; i < matches.size(); i++) {
                ...
                // if its found
                if (loc != string::npos) {
                    // highlight word if index matches highlight number
                    if (highlight == i){
